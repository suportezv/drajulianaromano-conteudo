# {{MARCA}} Conteúdo Studio (memória persistente do projeto)

Este repositório é o **{{MARCA}} Conteúdo Studio**: edição e agendamento de conteúdo para as redes da **{{MARCA}}**. Estúdio da agência com infraestrutura compartilhada; o posicionamento é o desta marca.
...
- Quando citar a criadora ou criador: sempre a credencial completa **"{{CREDENCIAL}}"**.
- Palavrão em vídeo **se bipa, não se corta**.
- Loudness final: **-14 LUFS**.

## Working dirs

- Estúdio: este repo (symlink `~/{{REPO}}` aponta para cá). Projetos em `projects/<nome>/`.
