# {{MARCA}} Conteúdo Studio: FRAMEWORK

Estúdio de edição e agendamento para as redes da **{{MARCA}}**. Framework compartilhado da agência, adaptado para esta marca.
...
- **PENDENTE**: persona, tom de voz e posicionamento da {{MARCA}} (definir no primeiro briefing).
- CTA padrão: **{{CTA}}**.
- Quando citar a criadora ou criador: credencial sempre completa, **"{{CREDENCIAL}}"**.

### REGRAS INEGOCIÁVEIS

1. **Nunca usar travessão em texto público.** Reescrever a frase.
2. Credencial sempre completa: **"{{CREDENCIAL}}"**.
