...
- **`scripts/`**: setup e validação do ambiente (Linux/cloud), o wrapper `hyperframes.sh` (browser e proxy certos em qualquer shell), o `vendor_gsap.sh` (GSAP local, exigido pelo `check`) e as ferramentas genéricas de produção (abaixo).
